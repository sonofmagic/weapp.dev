...
test('renders the bilingual ecosystem home with valid metadata', async ({ page }) => {
  ...
  await expect(page.locator('.home-adjacent-projects article').first()).toHaveAttribute('aria-describedby', 'home-adjacent-description-weapp-sqlite home-adjacent-status-weapp-sqlite home-adjacent-proof-weapp-sqlite home-adjacent-roadmap-weapp-sqlite')
  ...
  await expect(page.locator('#about li').first()).toHaveAttribute('aria-labelledby', 'about-step-title-1')
  await expect(page.locator('#about li').first()).toHaveAttribute('aria-describedby', 'about-step-body-1')
  ...
  for (const id of ['weapp-vite', 'weapp-tailwindcss', 'varo', 'weapp-sqlite', 'vite-plugin-taro']) {
    ...
    const describedBy = ['varo', 'weapp-sqlite'].includes(id)
      ? `home-project-description-${id} home-project-status-${id} home-project-proof-${id} home-project-readiness-${id}`
      : `home-project-description-${id} home-project-status-${id} home-project-proof-${id}`
    await expect(page.locator(`.home-project-row[data-project-id="${id}"]`)).toHaveAttribute('aria-describedby', describedBy)
  }
  await expect(page.locator('#projects .home-lab')).toHaveCount(0)
...
test('renders the bilingual pricing and delivery page', async ({ page }) => {
  ...
  await expect(page.locator('.pricing-sponsor-button').first()).toHaveAttribute('aria-label', '去 GitHub 谈赞助: 普通支持')
...
test('provides a working mobile navigation menu', async ({ page }) => {
  ...
  for (const [route, navigationName, menuLabel] of [['/', '移动端导航', '打开导航'], ['/en/', 'Mobile navigation', 'Open navigation']] as const) {
    await page.goto(route)
    const menu = page.locator('[data-mobile-menu] summary')
    await expect(menu).toHaveAttribute('aria-expanded', 'false')
    await expect(menu).toHaveAttribute('aria-controls', 'mobile-navigation')
    await menu.click()
    await expect(menu).toHaveAttribute('aria-expanded', 'true')
    await expect(menu).toHaveAttribute('aria-label', menuLabel === '打开导航' ? '关闭导航' : 'Close navigation')
    ...
    await expect(menu).toHaveAttribute('aria-expanded', 'false')
    await expect(menu).toHaveAttribute('aria-label', menuLabel)
...
test('opens analytics preferences directly from the privacy page', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: '统计偏好' })).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(page.getByRole('dialog', { name: '统计偏好' })).toBeHidden()
  await expect(page.getByRole('button', { name: '打开统计偏好' })).toBeFocused()
  await page.getByRole('button', { name: '打开统计偏好' }).click()
